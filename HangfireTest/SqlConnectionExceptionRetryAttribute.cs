...
{
    public class SqlConnectionExceptionRetryAttribute : JobFilterAttribute, IElectStateFilter, IApplyStateFilter
    {
        private const int DefaultRetryAttempts = 10;
        private int _attempts;
        private static readonly ILog Logger = LogProvider.GetCurrentClassLogger();

        public SqlConnectionExceptionRetryAttribute()
        {
        ...
        public void OnStateElection(ElectStateContext context)
        {
            ...
            //do not retry for exceptions other than SqlException
            if (failedState != null && failedState.Exception != null && !IsSqlConnectionTimeoutException(failedState.Exception))
            {
                Attempts = 0;
                OnAttemptsExceeded = AttemptsExceededAction.Fail;
            }
        ...
        private bool IsSqlConnectionTimeoutException(Exception exception)
        {
            // Check if the exception or any inner exception is a SQL connection timeout exception
            if (exception is SqlException sqlException)
            {
                foreach (SqlError error in sqlException.Errors)
                {
                    if (error.Number == -2) // SQL connection timeout error number
                    {
                        return true;
                    }
                }
            ...
            {
                return IsSqlConnectionTimeoutException(exception.InnerException);
            }
